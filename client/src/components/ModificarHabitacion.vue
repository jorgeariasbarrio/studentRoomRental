<template>
  <div class="card" style="width: 80%; margin-left: 10%">
    <div class="card" style="width: 70%; margin: 1rem auto">
      ...
      <div style="margin-bottom: 5%">
        <button
          ...
          v-on:click="cambiarEstadoHabitacion"
        >
          Ocultar habitación
        </button>
        <button
        ...
        <div
          class="form-floating"
          v-if="
            habitacion.estadoHabitacion === 'ALQUILADA' && linkReview !== null
          "
        ...
        >
          <input
            ...
            disabled
          />
          <label for="linkReview">Link para realizar review</label>
        </div>
      </div>
      <div class="container">
        <div class="row">
    ...
    <button class="btn btn-secondary" v-on:click="cancelarModificar">
      Cancelar
    </button>
    <button
      class="btn btn-success"
      type="button"
      v-on:click="modificarHabitacion"
    >
...
export default {
  ...
  data() {
    return {
      ...
      opcionesHabitacion: [],
      imagenesHabitacion: [],
      //para poder borrar las imagenes antes de subir las nuevas
      imagenesCopia: [],
      linkReview: null,
    };
  },
  ...
  methods: {
    async getOpcionesHabitacion() {
      ...
      await this.getHabitacion();
    },
    async getHabitacion() {
      this.habitacion = await HabitacionRepository.findHabitacion(
        this.$route.params.id
      );
      if (this.habitacion.fianza === 0) {
        this.habitacion.sFianza = true;
      }
      if (this.habitacion.gastosCompartidos === 0) {
        this.habitacion.gastosCInc = true;
      }
      for (let i = 0; i < this.habitacion.opciones.length; i++) {
        for (let x = 0; x < this.opcionesHabitacion.length; x++) {
          if (
            this.opcionesHabitacion[x].opcionObjeto.id ===
            this.habitacion.opciones[i].id
          ) {
            this.opcionesHabitacion[x].seleccionada = true;
          }
        }
      }
      await this.getImagenesHabitacion();
    },
    async getImagenesHabitacion() {
      let imagenes = await HabitacionRepository.cargarImagenes(
        this.$route.params.id
      );
      this.imagenesCopia = imagenes;
      for (let i = 0; i < imagenes.length; i++) {
        ...
        // Esto es necesario para cargar los archivos que recuperamos de la bd.
        //blob es el formato en el que podemos recuperar el archivo, y para pasarlo a File necesitamos
        // la linea que lo convierte en un File.
        let blob = await response.blob();
        let imagen = new File([blob], imagenes[i].nombre);
        const reader = new FileReader();
        reader.readAsDataURL(imagen);
        reader.addEventListener("load", (e) => {
          let obj = {
            imagenFR: e.target.result, // imagen en formato base64 para img en html
            imagenFile: imagen, // archivo imagen
            portada: imagenes[i].portada,
            idImagen: imagenes[i].idImagen, // si va ser la portada
          };
          this.imagenesHabitacion.push(obj);
        });
      }
      await this.getLinkReviewPendiente();
    },
    async getLinkReviewPendiente() {
      let reviews = await ReviewRepository.getReviewsByHabitacion(
        this.habitacion.id
      );
      if (
        reviews.length !== 0 &&
        reviews[reviews.length - 1].completa === false
      ) {
        this.linkReview = reviews[reviews.length - 1].link;
        this.linkReview = "http://localhost:1234/reviews/" + this.linkReview;
      } else {
        this.linkReview = null;
      }
    },
    cancelarModificar() {
      this.$router.go(-1);
    ...
    async cambiarEstadoHabitacion() {
      if (this.habitacion.estadoHabitacion === "DISPONIBLE") {
        if (confirm("¿Has alquilado esta habitación?") === true) {
          this.$router.push({
            name: "habitacionAlquilada",
            params: { idHabitacion: this.$route.params.id },
          });
        } else {
          this.habitacion.estadoHabitacion = "OCULTA";
          await HabitacionRepository.cambiarEstadoHabitacion(
            this.habitacion.id
          );
        }
      } else if (this.habitacion.estadoHabitacion === "ALQUILADA") {
        await HabitacionRepository.cerrarAlquiler(this.habitacion.id);
        this.habitacion.estadoHabitacion = "DISPONIBLE";
      } else {
        this.habitacion.estadoHabitacion = "DISPONIBLE";
        await HabitacionRepository.cambiarEstadoHabitacion(this.habitacion.id);
      }
    },
    async modificarHabitacion() {
      const toast = useToast();
      let erroresFormulario = false;
      this.habitacion.opciones = [];
      ...
      if (erroresFormulario === true) {
        ...
        await HabitacionRepository.modificarHabitacion(
          this.habitacion,
          this.habitacion.piso.id
        );
        for (let i = 0; i < this.imagenesCopia.length; i++) {
          await HabitacionRepository.borrarImagen(
            this.$route.params.id,
            this.imagenesCopia[i].idImagen
          );
        }
        ...
        toast.success(
          "Se ha actualizado correctamente la infomación de la habitación",
          {
            position: "top-right",
            timeout: 5000,
            closeOnClick: true,
